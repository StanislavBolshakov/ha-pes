...
class Client:
    def __init__(self, token: str = None) -> None:
        ...
        self.rest_client = RestClient(token=token)
    ...
    def prepare_meter_data(self):
        ...
        data = (
            self.get_account_data(result["account_id"])
            .get("indicationInfo")
            .get("subServices")
        )
        ...
        for meter in data:
            if meter.get("scale") == "DAY":
                ...
                raise UnsupportedArgumentError(
                    f'Unexpected data in meter scale, got "{meter.get("scale")}"'
                )
        ...
        if not isinstance(result["account_id"], int):
            raise UnsupportedArgumentError(
                f'Unexpected Account ID type, got "{type(result["account_id"])}"'
            )
        if not all(
            isinstance(i, str)
            for i in [
            ...
            raise UnsupportedArgumentError(
                "Unexpected data in Account Num, Meter ID or Meter Number"
            )
    ...
    def update_meter_counters(self, values: list = None):
        ...
        if len(values) != 2:
            raise UnsupportedMeterType(
                f"Only two tarrifs are supprted, got {len(values)}"
            )
        ...
        for value in values:
            if not isinstance(value[0], int):
                raise UnsupportedArgumentError(
                    f'New meter value must be INT, got "{type(value[0])}'
                )
            if value[1] not in ["DAY", "NIGHT"]:
                raise UnsupportedArgumentError(
                    f'Supported values are DAY or NIGHT, got "{value[1]}"'
                )
        ...
        payload = {
            "accountId": int(account_data["account_id"]),
            "newIndications": new_meter_values,
        }
